...
# Domeinen: abelsoftware123.com (hoofddomein) +
#           abelsoftware123.abelsoftware123.com (blijft ook werken)
# ===================================================================

PRIMARY_DOMAIN="abelsoftware123.com"
SECONDARY_DOMAIN="abelsoftware123.abelsoftware123.com"
...
JAVA_FILES="UserController.java AdminController.java SecurityConfig.java DataInitializer.java CustomUserDetailsService.java UpdateProfileRequest.java UserProfileDTO.java User.java UserRepository.java UserService.java RegistratieApplication.java"
...
# --- CustomUserDetailsService.java: ONTBRAK VOLLEDIG - dit is waarom login altijd faalde ---
# Zonder deze bean weet Spring Security niet hoe het gebruikers uit de Neon-database
# moet ophalen en verifieren, en wijst het elk wachtwoord af.
if [ ! -f "$PKG_DIR/CustomUserDetailsService.java" ]; then
cat > "$PKG_DIR/CustomUserDetailsService.java" << 'JAVAEOF'
package com.abelsoftware123.registratie;
...
JAVAEOF
    echo "   java (auto-gegenereerd): CustomUserDetailsService.java"
fi
...
# -------------------------------------------------------------
# 7. NGINX SERVER BLOCKS TOEVOEGEN (raakt bestaande sites niet aan)
# -------------------------------------------------------------
echo ">> Nginx server block wordt aangemaakt voor ${PRIMARY_DOMAIN}..."
...
if [ ! -L "/etc/nginx/sites-enabled/${PRIMARY_DOMAIN}" ]; then
    ln -s /etc/nginx/sites-available/${PRIMARY_DOMAIN} /etc/nginx/sites-enabled/${PRIMARY_DOMAIN}
fi

echo ">> Nginx server block wordt aangemaakt voor ${SECONDARY_DOMAIN}..."

cat > /etc/nginx/sites-available/${SECONDARY_DOMAIN} <<EOF
server {
    listen 80;
...
if [ ! -L "/etc/nginx/sites-enabled/${SECONDARY_DOMAIN}" ]; then
    ln -s /etc/nginx/sites-available/${SECONDARY_DOMAIN} /etc/nginx/sites-enabled/${SECONDARY_DOMAIN}
fi
...
# -------------------------------------------------------------
# 8. GRATIS SSL VIA LET'S ENCRYPT (CERTBOT) - VOOR BEIDE DOMEINEN
# -------------------------------------------------------------
...
echo ">> SSL certificaat wordt aangevraagd voor ${PRIMARY_DOMAIN} en www.${PRIMARY_DOMAIN}..."
certbot --nginx -d ${PRIMARY_DOMAIN} -d www.${PRIMARY_DOMAIN} --non-interactive --agree-tos -m admin@${PRIMARY_DOMAIN} --redirect || \
    echo "!! Certbot kon geen certificaat regelen voor ${PRIMARY_DOMAIN}. Waarschijnlijk moet je DNS nog even doorpropageren (kan uren duren). Probeer later handmatig: sudo certbot --nginx -d ${PRIMARY_DOMAIN} -d www.${PRIMARY_DOMAIN}"

echo ">> SSL certificaat wordt aangevraagd voor ${SECONDARY_DOMAIN}..."
certbot --nginx -d ${SECONDARY_DOMAIN} --non-interactive --agree-tos -m admin@${PRIMARY_DOMAIN} --redirect || \
    echo "!! Certbot kon geen certificaat regelen voor ${SECONDARY_DOMAIN}. Mogelijk had die al een certificaat van een vorige run - dat is geen probleem."

echo "======================================================"
echo " KLAAR!"
echo " Backend service: systemctl status ${SERVICE_NAME}"
echo " Website zou nu bereikbaar moeten zijn op:"
echo " https://${PRIMARY_DOMAIN}  (hoofddomein)"
echo " https://${SECONDARY_DOMAIN}  (blijft ook werken)"
echo " https://${DOMAIN}"
echo "======================================================"
